...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  $$unsubscribe_user();
  $$unsubscribe_page();
  return `<section><div class="${"user svelte-1yhyupc"}"><img${add_attribute("src", userProfile?.avatar?.url, 0)} alt="${"profile"}" class="${"svelte-1yhyupc"}">
		<div class="${"user-stats svelte-1yhyupc"}"><span>${escape(userProfile?.username)}</span>
			<div><span style="${"color: var(--ins-color);;"}">${escape(userProfile?.wins)}</span>
				/
				<span style="${"color: var(--color);"}">0</span>
				/
				<span style="${"color: var(--del-color);;"}">${escape(userProfile?.losses)}</span></div></div></div></section>
...
//# sourceMappingURL=_page.svelte-c2d677c6.js.map
